...
export async function generateMetadata({ params }) {
  ...
  const res = await GetData(`/get-content/${_id}?type=national-park`);
  const data = res?.data;
  ...
  return {
    ...
    openGraph: {
      title: data?.title || "Default Title",
      description: data?.description || "Default Description",
      url: `https://www.nepalmemorable.com/services/national-park/${id}`,
      type: "article",
      images: [
        {
          url: coverImage,
...
const page = async ({ params }) => {
  ...
  return (
    <DetailsPage
      data={data}
      location={`https://www.nepalmemorable.com/services/national-park/${id}`}
    />
  );
};
